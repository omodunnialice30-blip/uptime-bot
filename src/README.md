...
3.er → New File → name— orchestrates the above: runs checks, tracks consecutive failures per target inname it README.md ( and only alerts once a target has failedp-level uptime-bot fotimes in a row (avoids alert spam from single blips). Also sends a "recovered" message when a target comes back up.
